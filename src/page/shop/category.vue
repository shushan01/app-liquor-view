    ...
    <div>
        ...
        <div class="categoryBody">
            ...
            <div class="rootList" style="width:20%;">
                <load-more ref="rootScroll" style="height:85%;">
                    <ul class="rootListcontent">
                        <li :class="selectedRoot === item.Id ? 'active' : ''" @click="/*rootScrollTo(item)*/"
                            v-for="(item,index) in categoryBody.categoryRoot"
                            v-if="categoryBody.categoryRoot!=null">{{item.name}}
                        </li>
                    </ul>
                </load-more>
            </div>
            ...
            <div class="jd-category-content" style="width: 85%;">
                <load-more :loadMoreIconVisible="false" ref="loadMore"
                           style="width: 100%;height:85%;left:initial;right:0;">
                    <div class="jd-categoryContent">
                        <div class="categoryContentBox"
                             v-if="categoryBody.categoryList!=null && categoryBody.categoryList.length>0">
                            <div class="categorytItem" v-for="(item,index) in categoryBody.categoryList" :key="index"
                                 @click="$router.push({path: '/searchRusult',query: {categoryId:item.Id}})">
